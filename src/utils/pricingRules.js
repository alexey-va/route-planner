...
export function getPricingRules() {
    ...
    return [
        ...
        },
        {
            ...
            items: vehicleRules.flatMap((rule) => [
                rule.title,
        ...
        },
        {
            title: 'Коминтерн',
            items: [
                'Скидка 50% в среду или пятницу — только для Газели 1.5т и 2т',
                `Минимальная стоимость после скидки: ${config.kominter_min_price} руб`,
                'Бесплатная доставка в Коминтерне не действует',
                'При скидке Коминтерна другие надбавки (время, выходные) не применяются'
            ]
        }
    ];
}
